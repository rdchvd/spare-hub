    ...
    >
      <Card className="overflow-hidden border-border/70 transition-all group-hover:border-accent/60 group-hover:shadow-md h-full">
        <div className="aspect-[4/3] bg-gradient-to-br from-secondary to-muted flex items-center justify-center relative">
          <span className="text-6xl select-none transition-transform group-hover:scale-110">{listing.emoji}</span>
        ...
        <CardContent className="p-4">
          ...
          <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
            ...
            <span className="flex items-center gap-1"><Star className="h-3 w-3 fill-[color:var(--gold)] text-[color:var(--gold)]" /> {listing.rating} <span className="opacity-60">({listing.reviews})</span></span>
          </div>
          <div className="mt-4 flex items-end justify-between">
            <div>
              ...
              <div className="text-xs text-muted-foreground truncate max-w-[10rem]">{listing.seller}</div>
            </div>
            <Button
              size="sm"
              variant="secondary"
